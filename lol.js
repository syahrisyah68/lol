...
const vcard = 'BEGIN:VCARD\n'  // Jangan di ubah biar ga error
            ...
            + 'TEL;type=CELL;type=VOICE;waid=6285893123964:+62 85893123964\n'  // Ganti jadi nomormu, tapi jangan ubah polanya
...
async function starts() {
    ...
    fs.existsSync('./zharzx.json') && lolteam.loadAuthInfo('./zharzx.json')
    if (apikey == "") {
   ...
   if (cr == "") {
    ...
    fs.writeFileSync('./zharzx.json', JSON.stringify(lolteam.base64EncodedAuthInfo(), null, '\t'))
 ...
 if (isCmd) cmdadd()
		...
		case 'daftar':
               ...
               zharzx = fs.readFileSync('lib/zharzx.jpg')
                veri = sender
                if (isGroup) {
                    addRegisteredUser(sender, namaUser, umurUser, time, serialUser)
                    await lolteam.sendMessage(from, zharzx, image, {quoted: lol, caption: ind.registered(namaUser, umurUser, serialUser, time, sender)})
		...
		case 'setapikey':
					...
					if (sender.split("@")[0] != owner) return reply("Command only for owner bot")
					apikey = args[0]
					reply(`Apikey berhasil di ubah menjadi : muzharzain`)
					break 
		case 'toimg2':
		if (!isRegistered) return reply(ind.noregis())
		...
		if (!isRegistered) return reply(ind.noregis())
                ...
                cf = `${body.slice(8)}`
                    ...
                    if ((isMedia && !lol.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        request({
                            ...
                            fs.writeFileSync(file_name, body, "binary")
                            ini_buff = fs.readFileSync(file_name)
                            await lolteam.sendMessage(from, ini_buff, image, { quoted: lol }).then(() => {
                                fs.unlinkSync(file_name)
                            })
                        });
                    } else {
                ...
                case 'wallnime':
                    ...
                    getBuffer(`https://api.lolhuman.xyz/api/random/${command}?apikey=${apikey}`).then((gambar) => {
                        lolteam.sendMessage(from, gambar, image, { quoted: lol })
                    })
                    break
case '1977':
                case 'aden':
                ...
                case 'xpro2':
                    ...
                    if ((isMedia && !lol.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        request({
                            ...
                            fs.writeFileSync(file_name, body, "binary")
                            ini_buff = fs.readFileSync(file_name)
                            lolteam.sendMessage(from, ini_buff, image, { quoted: lol }).then(() => {
                                fs.unlinkSync(file_name)
                            })
                        });
                    } else {
		...
		case 'tebakbendera':
			...
			if (!isRegistered) return reply(ind.noregis())
				...
				case 'namaninja': 
				if (!isRegistered) return reply(ind.noregis())
				if (args.length < 1) return reply(`Contoh: ZHARZXCHANNEL`)
